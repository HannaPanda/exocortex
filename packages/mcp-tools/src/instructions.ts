...
Beim Schreiben: exo_page_write mit mode "append" ergänzt, "replace" ersetzt die ganze Seite (und gilt auch, wenn mode fehlt). Für eine einzelne Stelle einer langen Seite gibt es exo_page_block_update, exo_page_patch und exo_page_section_write; die lassen den Rest der Seite unangetastet. Ist ein Abschnitt zu einem eigenen Thema geworden, verschiebt ihn exo_page_extract_section mit einem Aufruf auf eine eigene Seite, statt die Seite weiter wachsen zu lassen. Ab einer gewissen Größe sagt die Antwort dazu, wie groß die Seite geworden ist; ab einer zweiten lehnt sie weiteres Anhängen ab und nennt den Abschnitt, der auf eine eigene Seite gehört. Kürzen, Umschreiben und das Ändern eines einzelnen Abschnitts bleiben in beiden Fällen möglich. Vor jedem Schreibvorgang wird ein Snapshot angelegt.`;
