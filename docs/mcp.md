...
| `exo_workspace_reorder`            | yes      | yes         | `PATCH /api/workspaces/order` (the caller's own order; named ids first, the rest keep theirs)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `exo_page_read`                    | no       | no          | `GET /api/documents/:documentId/export/markdown`, with a budget on it: above 10,000 characters the route answers with the page's **map** instead of its text, and `view` says which came back (issue #118). `transclusions: 'text'` puts the embedded sources' text in place of the `:::transclusion` references, for an export that leaves this deployment. `includeBlockIds: true` writes each block's identifier after it as `^id`, which is the address the three narrow writes take (issue #111). The answer ends with `Revision dieser Seite: …`, the page's `yjsUpdatedAt` and the only value `expectedYjsUpdatedAt` may be given (issue #120)                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `exo_page_block_read`              | no       | no          | `GET /api/documents/:documentId/fragment` -- one block of a page instead of the whole page (issue #78, ADR-045). Without `blockId` the page's map: its sections with heading, size and block identifier. With one, that section's Markdown, or -- when the section is itself over the budget -- the map of _it_, down to block windows (`blockId` plus `toBlockId`) for a part that has no inner headings. `blocks: true` still returns the flat list of every addressable block with a preview. A heading addresses its whole section, which is what leaving `toBlockId` out asks for: the same identifier in both ends is that one block. On a heading that would be the heading line alone, so the tool reads the section instead and says above the answer that it widened the request; the range semantics underneath, and what the browser sees, are unchanged. Every answer ends with the page's revision, so one section read is enough to write that section back (issue #120). The way to read a long page one section at a time, and the address a `:::transclusion` block embeds |
...
| `exo_share_mine`                   | no       | no          | `GET /api/me/outgoing-shares` -- every grant this account made, in every workspace it is still a member of, with the workspace named and whether it may still withdraw it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `exo_context_compile`              | no       | no          | `GET /api/context?q=&workspaceIds=&maxChars=&maxSources=&perSourceMaxChars=` -- verbatim passages from every readable workspace (the memory area only when named), packed into the budget with provenance (issue #110, ADR-061)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `exo_work_item_list`               | no       | no          | `GET /api/workspaces/:workspaceId/work-items?status=&assignee=&requester=&parentId=&open=&limit=` -- delegated work (issue #138, ADR-066), open items first; `open` defaults to true                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `exo_work_item_get`                | no       | no          | `GET /api/work-items/:workItemId` -- goal, criteria, context and result pages, children, linked runs with cost, budget, history                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `exo_work_item_create`             | yes      | no          | `POST /api/workspaces/:workspaceId/work-items` -- the caller is the requester; the assignee is a member, an agent account or `{kind:"assistant"}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `exo_work_item_update`             | yes      | no          | `PATCH /api/work-items/:workItemId` -- flat fields; how an agent reports back (`status`, `statusReason`, `result`, `resultDocumentIds`, criteria). The assignee may do this without being a MEMBER                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `exo_work_item_note`               | yes      | no          | `POST /api/work-items/:workItemId/notes` -- a line in the history, never edited                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `exo_work_item_start_run`          | yes      | no          | `POST /api/work-items/:workItemId/runs` -- a built-in AI run in a conversation of its own, from the newest working state unless `fromCheckpoint` says `none` or names another (ADR-069); `modelSlug` may differ from earlier runs; refused on a closed item or a spent budget                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `exo_work_item_checkpoint`         | yes      | no          | `POST /api/work-items/:workItemId/checkpoints` -- where the work stands (issue #142, ADR-069): summary, plan with step status, assumptions, findings, last action, next step, pages made and used; fields left out carry the previous checkpoint forward                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `exo_work_item_checkpoints`        | no       | no          | `GET /api/work-items/:workItemId/checkpoints?limit=` -- the recorded working states, newest first, with pages marked when they changed since and the decisions they waited on                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `exo_work_item_delete`             | yes      | yes         | `DELETE /api/work-items/:workItemId` -- irreversible, the history goes with it; requester or workspace admin. Children are detached                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `exo_attention_list`               | no       | no          | `GET /api/attention?scope=&status=&workspaceId=&workItemId=&conversationId=&kind=&limit=` -- what waits on a person across workspaces (issue #139, ADR-067); `for_me` by default, `raised_by_me` is how an agent finds its answers, `conversationId` the checkpoints a chat's runs raised (#140)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `exo_attention_get`                | no       | no          | `GET /api/attention/:attentionItemId` -- one item with who settled it and how                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `exo_attention_request`            | yes      | no          | `POST /api/workspaces/:workspaceId/attention` -- ask a person; on a work item a blocking question (default) moves it to `waiting_for_human` and pauses the built-in run that asked (ADR-068); `workState` is handed back with the answer, an approval names its `action` and may bind `subjectPages`, `review` moves the item into review; `key` is idempotent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `exo_attention_resolve`            | yes      | no          | `POST /api/attention/:attentionItemId/resolve` -- `optionId` and/or `note`; a system option is the work item change it stands for                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `exo_attention_withdraw`           | yes      | no          | `POST /api/attention/:attentionItemId/withdraw` -- the asker takes a request back; system items cannot be withdrawn                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `exo_changeset_propose`            | yes      | no          | `POST /api/workspaces/:workspaceId/changesets` or `POST /api/changesets/:changesetId/changes` -- propose one page change instead of writing it (issue #141, ADR-070); resolved and diffed at once, the page stays as it is. Without `changesetId` it starts a draft. Class `proposal`: allowed in the propose mode and after foreign content                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `exo_changeset_submit`             | yes      | no          | `POST /api/changesets/:changesetId/submit` -- hand a draft in; frozen from here. On a work item the work moves into review and the built-in run ends its turn (`pausesRun`); the decision carries it on                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `exo_changeset_remove_change`      | yes      | no          | `DELETE /api/changesets/:changesetId/changes/:changeId` -- take a change out of one's own draft                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `exo_changeset_discard`            | yes      | yes         | `DELETE /api/changesets/:changesetId` -- throw one's own draft away; a handed-in set stays as the record                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `exo_changeset_get`                | no       | no          | `GET /api/changesets/:changesetId` -- every change with its diff, whether it still fits its page, and who decided what                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `exo_changeset_list`               | no       | no          | `GET /api/workspaces/:workspaceId/changesets?state=&workItemId=&proposedBy=&limit=` -- `open` by default                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `exo_changeset_apply`              | yes      | no          | `POST /api/changesets/:changesetId/apply` -- MCP only: apply chosen or all pending changes through the ordinary writes at the expected revision; a page that moved marks the change stale                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `exo_changeset_reject`             | yes      | no          | `POST /api/changesets/:changesetId/reject` -- MCP only: reject chosen or all pending changes with a note for the proposer                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `exo_attachment_upload_ticket`     | yes      | no          | `POST /api/workspaces/:workspaceId/attachments/upload-tickets` (ADR-064) -- a one-time `uploadUrl`, ten minutes, that a script POSTs a local file to (`POST /api/attachments/upload/:secret`, public). The text carries a ready `curl` line. MCP only: the built-in AI has no files to send                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `exo_attachment_upload_ticket_get` | no       | no          | `GET /api/workspaces/:workspaceId/attachments/upload-tickets/:ticketId` -- open, used (with `embedUrl`) or expired; only the caller's own tickets. MCP only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `exo_rules_load`                   | no       | no          | `GET /api/documents/:documentId/export/markdown` (capped at 60,000 chars). An identifier that leads nowhere is answered rather than thrown: the miss names the character count when it differs from 24 and points at `exo_rules_list`, because the argument is always 24 characters copied by hand out of the system prompt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
There is exactly one revision of a page, `DocumentContent.yjsUpdatedAt`, and
since [issue #120](https://github.com/HannaPanda/exocortex/issues/120) every
read publishes it: `GET /export/markdown` and `GET /fragment` both carry
`yjsUpdatedAt`, and `exo_page_read` and `exo_page_block_read` end their text
with `Revision dieser Seite: …`. That line is the only source for
`expectedYjsUpdatedAt`. The frontmatter's `updatedAt` is the `document` row's
timestamp and a different clock, which is what agents sent while no read
returned the revision: every read-then-write came back
`document_content_conflict` and the only way through was to drop the guard. A
conflict now names the revision to retry with, and says outright when the value
it was handed was the frontmatter one.
...
1. Upload it, with the tool that fits where the file is:
   - **on your own disk** (a photo from a chat, a PDF, a screenshot):
     `exo_attachment_upload_ticket`. It answers with a one-time `uploadUrl`,
     valid for ten minutes, and your script sends the file there
     (`curl -F "file=@/pfad/zur/datei" <uploadUrl>`); the JSON answer carries
     `embedUrl`. `exo_attachment_upload_ticket_get` reads the ticket's state
     when the script's output is out of sight
     ([ADR-064](adr/ADR-064-a-local-file-arrives-through-an-upload-ticket.md)).
   - **already public on the web**: `exo_attachment_upload_url`, which fetches
     it here through the same address check web research uses
     ([ADR-033](adr/ADR-033-web-research-is-two-rest-calls.md)).
   - **a few bytes you generated yourself**: `exo_attachment_upload` with
     Base64. The model writes every byte into the call, so this is not the way
     for a photograph.

   Never put a file on another server so that it can be fetched: that mixes
...
`warnings`. Creating a page with Markdown (`exo_page_create`) answers with the
same warnings since 2026-09-25; before that only `exo_page_write` did.
...
`ApiToken.scopes` is enforced by `TokenScopeGuard` (`read`, `propose`,
`write`, `admin`, cumulative; an empty list grants nothing). `propose` (issue
#141, ADR-070) may report on delegated work and propose changesets, never write
a page or decide a proposal: the routes are classed by `requestClassFor` in
`packages/auth/src/api-token.ts`, and the built-in AI in the propose or
read-only mode is held to the same classes through a claim signed into its
service token. The HTTP endpoint passes an `exo_`
...
Three tools read, and they are deliberately not interchangeable:
`exo_search` finds pages, `exo_page_read` loads a named one (or its map, ADR-056),
and `exo_context_compile` answers a question with the passages out of several
pages that bear on it (issue #110,
[ADR-061](adr/ADR-061-context-is-compiled-from-passages-not-pages.md)). The
last one is the first call for a knowledge question: one request instead of a
search and a string of reads.

It searches every workspace the account is a member of, or the ones named in
`workspaceIds`; the memory area only when it is named, because `recall` is the
tool for session notes. Passages come back verbatim with title, path,
workspace, date and, where known, the heading and its block id, so
`exo_page_block_read` can open the section. `maxChars` is a hard ceiling on the
rendered text, `maxSources` caps how many pages contribute and
`perSourceMaxChars` (a third of the budget by default) how much one page may
take. `truncated` says when something relevant was left out or cut, and
`stages` says whether the semantic half answered; without it the answer is
full-text alone, never an error.
...
and nothing else. Nine tools declare `irreversible: true` and a test pins the
...
`exo_render_template_delete`, `exo_user_delete`, `exo_work_item_delete` and
`exo_share_create`. An ordinary write runs on the first call.
...
subprocess. Neither can _narrow_ it below the nine irreversible calls.
