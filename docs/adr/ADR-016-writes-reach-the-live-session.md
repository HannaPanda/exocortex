...
- Authorization is _not_ taken from the token. The token says who is writing;
...
- `append` and `prepend` insert _only the incoming nodes_. Nothing existing is
...
- The overwrite window is closed for a loaded document, and for an unloaded one
  ...
  new state from the database. A session that connects _during_ the exchange
