...
this ADR settles is what a database *is* in terms already in the schema, and
what a row *is*, before any API or UI is built on top.
...
code from the property's *loaded* type, never from the request — the one
